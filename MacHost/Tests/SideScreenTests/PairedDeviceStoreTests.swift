...
final class PairedDeviceStoreTests: XCTestCase {
    ...
    func testUpsertAdds() {
        ...
        store.upsert(id: "device-1", name: "Pixel 9", deviceSecret: secret, lastConnected: Date(timeIntervalSince1970: 1000))
        XCTAssertEqual(store.all().count, 1)
        XCTAssertEqual(store.all().first?.id, "device-1")
        XCTAssertEqual(store.all().first?.name, "Pixel 9")
        XCTAssertEqual(store.deviceSecret(id: "device-1"), secret)
    }

    func testUpsertUpdatesExisting() {
        let store = freshStore()
        store.upsert(id: "device-1", name: "Pixel 9", lastConnected: Date(timeIntervalSince1970: 1000))
    ...
    func testRevokePreventsDeviceUse() {
        let store = freshStore()
        store.upsert(id: "device-1", name: "Pixel 9", lastConnected: Date())
    ...
    func testRoundTripJSON() {
        ...
        storeA.upsert(id: "device-1", name: "Pixel 9", lastConnected: Date(timeIntervalSince1970: 1000))
        let storeB = PairedDeviceStore(defaults: defaults)
        XCTAssertEqual(storeB.all().first?.id, "device-1")
        XCTAssertEqual(storeB.all().first?.name, "Pixel 9")
    }
